...
class TestCreateActivity:
    ...
    def test_falls_back_to_search_on_duplicate(self):
        from src.handlers.timesheet import _create_activity

        client = MagicMock()
        client.post.side_effect = TripletexApiError(ApiError(status=409, message="Already exists"))
        client.get.return_value = sample_api_response(values=[{"id": 5, "name": "Consulting"}])
        result = _create_activity(client, "Consulting")
        assert result == {"id": 5}

    def test_returns_zero_id_when_not_found(self):
        from src.handlers.timesheet import _create_activity
...
class TestLogTimesheetHandler:
    @patch("src.handlers.timesheet.resolve_employee", return_value={"id": 1})
    def test_happy_path_minimal(self, _mock_emp):
        client = MagicMock()
    ...
    @patch("src.handlers.timesheet.resolve_customer", return_value={"id": 2})
    @patch("src.handlers.timesheet.resolve_employee", return_value={"id": 1})
    def test_with_project_and_activity(self, _mock_emp, _mock_cust):
        client = MagicMock()
        client.get.return_value = sample_api_response(values=[{"id": 1}])
    ...
    @patch("src.handlers.timesheet.resolve_customer", return_value={"id": 2})
    @patch("src.handlers.timesheet.resolve_employee", return_value={"id": 1})
    def test_generates_invoice_with_hourly_rate(self, _emp, _cust, _bank):
        client = MagicMock()
        client.get.return_value = sample_api_response(values=[{"id": 1}])
    ...
    @patch("src.handlers.timesheet.resolve_employee", return_value={"id": 1})
    def test_timesheet_entry_failure(self, _mock_emp):
        client = MagicMock()
        client.get.return_value = sample_api_response(values=[{"id": 1}])
    ...
    @patch("src.handlers.timesheet.resolve_employee", return_value={"id": 1})
    def test_dict_project_and_activity(self, _mock_emp):
        client = MagicMock()
        client.get.return_value = sample_api_response(values=[{"id": 1}])
