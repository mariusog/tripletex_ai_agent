...
def main() -> None:
    ...
    print(f"{'=' * 80}")
    hdr = (
        f"{'Task Type':<30} {'Calls':>5} {'Opt':>4} {'Δ':>3}"
        f" {'Errs':>4} {'Time':>6} {'Lang':>4} {'Svc'}"
    )
    print(hdr)
    print(f"{'-' * 80}")
    ...
    for run in runs:
        ...
        lang = "?"
        pl = prompt.lower()
        if any(c in prompt for c in "äöüß"):
            lang = "DE"
        elif any(c in prompt for c in "ñ"):
            lang = "ES"
        elif any(c in prompt for c in "ção"):
            lang = "PT"
        elif any(c in prompt for c in "éèêë") and "le" in pl:
            lang = "FR"
        elif "opprett" in pl or "registrer" in pl:
            lang = "NO"
        elif "create" in pl or "register" in pl:
            lang = "EN"

        status = "ERR" if errors or run.get("status") == "error" else "OK"
        row = (
            f"[{status:>3}] {task:<25} {calls:>5} {optimal:>4} {delta:>3}"
            f" {errors:>4} {duration:>5.1f}s {lang:>4} {service}"
        )
        print(row)
    ...
    ranked = sorted(
        task_best.items(),
        key=lambda x: x[1].get("total_api_calls", 0) - OPTIMAL_CALLS.get(x[0], 0),
        reverse=True,
    )
