...
def refresh_token() -> None:
    ...
    try:
        ...
        with open("/tmp/gcloud_token", "w") as f:
            f.write(creds.token)
        os.system("gcloud config set auth/access_token_file /tmp/gcloud_token 2>/dev/null")
        os.system(f"gcloud config set project {PROJECT_ID} 2>/dev/null")
    ...
    for line in lines:
        ...
        if current_run and "Classified as task_type=" in line:
            ...
            if match:
                ...
                try:
                    current_run["params"] = json.loads(
                        match.group(2).replace("'", '"').replace("True", "true").replace("False", "false").replace("None", "null")
                    )
        ...
        if current_run and "Handler result" in line:
            ...
            if match:
                ...
                try:
                    current_run["result"] = json.loads(
                        match.group(5).replace("'", '"').replace("True", "true").replace("False", "false").replace("None", "null")
                    )
                except json.JSONDecodeError:
...
def main() -> None:
    ...
    parser.add_argument("--service", default="tripletex-agent-magnus", help="Cloud Run service name")
